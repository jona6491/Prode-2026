...
export default function PronosticosTab({ player, groupData, onSaved }) {
  const [selGrp, setSelGrp] = useState('A')
  const [predictions, setPredictions] = useState({}) // { 'A0': {l,v}, ... }
  const [saving, setSaving] = useState(false)
  const [saved, setSaved] = useState(player?.saved || false)
  const inputRefs = useRef({})

  // Load existing predictions if player already saved
  useEffect(() => {
    if (player?.saved) {
      setSaved(true)
  ...
  function updatePred(grp, idx, side, val) {
    ...
    setPredictions(prev => {
      const current = prev[key] || { l: '', v: '' }
      return { ...prev, [key]: { ...current, [side]: num } }
    })
  }
  ...
  function handleInput(grp, idx, side, val) {
    ...
    if (val !== '' && side === 'l') {
      const vKey = `v_${grp}${idx}`
      setTimeout(() => inputRefs.current[vKey]?.focus(), 0)
    }
  }
  ...
  function totalFilled() {
    return GROUP_KEYS.reduce((acc, g) => acc + countFilled(g), 0)
  }
  ...
  return (
    <div style={{display:'flex', flexDirection:'column', flex:1}}>
      <div className={`info-bar ${saved ? 'ok' : ''}`}>
        <span>{saved
          ? '✅ Pronósticos guardados. ¡Seguí el ranking en tiempo real!'
      ...
      <div style={{flex:1, overflowY:'auto', paddingBottom: saved ? 12 : 0}}>
        <div className="grp-block">
          <div className="grp-bar">
            <h3>
              Grupo {selGrp}
              <span className="cnt">{countFilled(selGrp)}/6</span>
            </h3>
            {!saved && (
          ...
          <div style={{padding:'8px 13px 0'}}>
                  ...
                  <tr key={row.team} className={i < 2 ? 'clf' : ''}>
                    ...
                    <td className="tl">
                      <div className="eq-cell">
                        {flagUrl(row.team)
                          ? <img src={flagUrl(row.team)} className="flag-img" alt={row.team} />
                          : <span>🏳</span>
                        }
                    ...
                    <td>{row.emp}</td>
                    <td>{row.perd}</td>
                    <td style={{color: row.dg > 0 ? 'var(--em)' : row.dg < 0 ? 'var(--red)' : 'var(--tx2)'}}>
            ...
            <div className="clf-note">
              <div className="clf-dot"></div>
              <span>Clasifica a 16vos de final</span>
            </div>
          </div>
                ...
                <div key={key} className="mrow">
                  ...
                  <div className="sbox">
                    {saved ? (
                      ...
                      <>
                        <input
                          className="sinp" type="number" min="0" max="20"
                          value={pred?.l ?? ''}
                          placeholder="0"
